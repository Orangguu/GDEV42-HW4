...
// TODO: check collision function and whether it should be moved or kept here
...
void Level::get_level_from_file(std::ifstream* level_data_file) {
  ...
  while (getline(*level_data_file, line)) {
    ...
    if (current_category_name == "GRID") {
      iss >> word;
      this->tilemap_size.x = std::stoi(word);
      iss >> word;
      this->tilemap_size.y = std::stoi(word);
      current_category_name = "";
      current_category_index = 0;
      continue;
    }
...
void Level::print_level_data() {
  ...
  std::cout << "Width: " << this->tilemap_size.x << std::endl;
  std::cout << "Height: " << this->tilemap_size.y << std::endl;
...
//     if (tileX < 0 || tileX >= tilemap_size.x || tileY < 0 || tileY >= grid_size.y)
//       return true;
